...
/**
 * doctor.mjs — Setup validation for uni-ops
...
function checkPrograms() {
  if (existsSync(join(projectRoot, 'programs.yml'))) {
    return { pass: true, label: 'programs.yml found' };
  }
  return {
    pass: false,
    label: 'programs.yml not found',
    fix: [
      'Run: cp templates/programs.example.yml programs.yml',
      'Then customize with your target universities and programs',
    ],
  };
}

function checkRecommenders() {
  if (existsSync(join(projectRoot, 'recommenders.yml'))) {
    return { pass: true, label: 'recommenders.yml found' };
  }
  return {
    pass: false,
    label: 'recommenders.yml not found',
    fix: [
      'Run: cp templates/recommenders.example.yml recommenders.yml',
      'Then fill in your referees',
    ],
  };
}

function checkAcademicRecord() {
  if (existsSync(join(projectRoot, 'academic-record.md'))) {
    return { pass: true, label: 'academic-record.md found' };
  }
  return {
    pass: false,
    label: 'academic-record.md not found',
    fix: 'Stub file should have been created by uni-ops scaffolding; if missing, create it manually with your per-semester GPA, course grades, and test scores',
  };
}

function checkExtracurriculars() {
  if (existsSync(join(projectRoot, 'extracurriculars.md'))) {
    return { pass: true, label: 'extracurriculars.md found' };
  }
  return {
    pass: false,
    label: 'extracurriculars.md not found',
    fix: 'Stub file should have been created by uni-ops scaffolding; if missing, create it manually',
  };
}
