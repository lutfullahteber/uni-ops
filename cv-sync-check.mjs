...
/**
 * cv-sync-check.mjs — Validates that the uni-ops setup is consistent.
 *
 * Checks:
 * 1. cv.md exists and is non-trivial
 * 2. academic-record.md exists and is non-trivial
 * 3. extracurriculars.md exists
 * 4. config/profile.yml exists, has required fields, not example data
 * 5. No hardcoded metrics in _shared.md or batch/batch-prompt.md
 * 6. article-digest.md freshness (if exists)
 */
...
// 2. Check academic-record.md
const academicPath = join(projectRoot, 'academic-record.md');
if (!existsSync(academicPath)) {
  errors.push('academic-record.md not found. Stub should be created by scaffolding; fill in per-semester GPA, course grades, and test scores.');
} else {
  const academicContent = readFileSync(academicPath, 'utf-8');
  if (academicContent.includes('X.XX / 4.0') || academicContent.includes('XX / XX')) {
    warnings.push('academic-record.md still contains stub placeholders ("X.XX"). Replace with your real numbers.');
  }
  if (academicContent.trim().length < 100) {
    warnings.push('academic-record.md is too short. Fill in your academic history.');
  }
}

// 3. Check extracurriculars.md
const extraPath = join(projectRoot, 'extracurriculars.md');
if (!existsSync(extraPath)) {
  warnings.push('extracurriculars.md not found. Highly recommended for admissions applications.');
}
...
if (!existsSync(profilePath)) {
  ...
  const requiredFields = ['full_name', 'email', 'degree_level', 'tuition_budget_max', 'total_annual_cap'];
