#!/usr/bin/env node
/**
 * analyze-patterns.mjs — Rejection Pattern Detector for uni-ops
 *
...
const APPS_FILE = existsSync(join(UNI_OPS, 'data/applications.md'))
  ? join(UNI_OPS, 'data/applications.md')
  : join(UNI_OPS, 'applications.md');
...
function analyze() {
  ...
  const enriched = entries.map(e => {
    ...
    const reportPath = reportMatch ? join(UNI_OPS, reportMatch[1]) : null;
