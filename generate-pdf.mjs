 ...
 *
 * Usage:
 *   node uni-ops/generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4]
 *
...
async function generatePDF() {
  ...
  // Parse arguments
  let inputPath, outputPath, format = 'a4', template = null;
  ...
  if (!inputPath || !outputPath) {
    console.error('Usage: node generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4] [--template=cv|sop]');
    process.exit(1);
  }

  // Validate template if supplied (informational only — the mode picks the HTML)
  if (template && !['cv', 'sop', 'motivation'].includes(template)) {
    console.error(`Invalid template "${template}". Use: cv | sop | motivation`);
    process.exit(1);
  }
  ...
  // Resolve font paths relative to uni-ops/fonts/
